...
export function renameList(command: RenameListCommand): void {
    ...
    if (!name) {
        throw new Error("Can't rename list: Name can't be empty");
    }

    if (activeListIsDefault) {
        throw new Error(`Can't rename default list '${activeList.name}'`);
    }
